...
export const zhCN = {
  ...
  style: {
    ...
    customMode: {
      title: '自定义风格',
      desc: '除内置 4 种风格外，可自由添加带独立 system prompt 的自定义风格。',
      addButton: '+ 新增自定义风格',
      empty: '还没有自定义风格。',
      idLabel: 'ID（不可修改）',
      idPlaceholder: '例如：my-style',
      idHint: '仅支持英文字母、数字和连字符。不能包含冒号（:）。',
      nameLabel: '显示名',
      namePlaceholder: '例如：杂记用',
      promptLabel: 'system prompt 正文',
      promptPlaceholder: '直接写入要传给 LLM 的指令……',
      delete: '删除',
      edit: '编辑',
      save: '保存',
      cancel: '取消',
      viewBuiltinPrompt: '查看默认 prompt',
      hideBuiltinPrompt: '收起',
      confirmDelete: '确定删除此自定义风格？',
      idEmpty: '请输入 ID。',
      idInvalid: 'ID 只能包含英文字母、数字和连字符。',
      nameEmpty: '请输入显示名。',
      promptEmpty: '请输入 system prompt。',
    },
    appOverride: {
      title: '按应用自动切换',
      desc: '在指定应用里听写时，自动套用对应的风格。匹配的是进程名的子串（例如 chrome、Discord.exe），不区分大小写；都不命中时使用默认风格。',
      addButton: '+ 新增规则',
      empty: '暂无规则。点击「+ 新增规则」开始添加。',
      patternLabel: '应用名（子串匹配）',
      patternPlaceholder: '例：chrome / Discord.exe',
      modeLabel: '应用的风格',
      delete: '删除',
      hint: '从上往下依次匹配，第一条命中的规则生效。匹配不区分大小写。',
    },
  },
  translation: {
    kicker: 'TRANSLATION',
    ...
    enable: {
      title: '启用翻译功能',
      hint: '关闭后，即使误触快捷键也不会启动翻译管线和 UI 浮层。',
    },
    working: {
      title: '工作语言',
  ...
  modal: {
    ...
    personalize: {
      ...
      fontMedium: '中',
      fontLarge: '大',
      fontFamilyLabel: 'UI 字体',
      fontFamilyDesc: '切换 App 内的显示字体。适用于中日文字体渲染冲突或自定义偏好。',
      fontFamilyAuto: '自动（默认优先级）',
      fontFamilyCustomLabel: '自定义（自由输入）…',
      fontFamilyCustomPlaceholder: '输入字体名（例：Yu Gothic UI）',
      fontInstalledLabel: '已安装字体 ({{count}})',
      fontLoadAll: '加载所有字体',
      fontReload: '重新加载 ({{count}})',
      fontLoading: '获取中…',
      fontLoadError: '无法获取字体列表（权限被拒或当前环境不支持）',
      quietLabel: '静默模式',
      quietDesc: '保留录音波形与处理点动画，抑制"翻译中""已输入 N 字""已取消"等瞬时文字提示。错误仍会显示。',
      quietAria: '切换静默模式',
      blur: '毛玻璃强度',
