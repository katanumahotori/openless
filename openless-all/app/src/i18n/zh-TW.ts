...
export const zhTW: typeof zhCN = {
  ...
  style: {
    ...
    customMode: {
      title: '自定義風格',
      desc: '除內建 4 種風格外，可自由新增帶獨立 system prompt 的自定義風格。',
      addButton: '+ 新增自定義風格',
      empty: '還沒有自定義風格。',
      idLabel: 'ID（不可修改）',
      idPlaceholder: '例如：my-style',
      idHint: '僅支援英文字母、數字和連字號。不可包含冒號（:）。',
      nameLabel: '顯示名稱',
      namePlaceholder: '例如：雜記用',
      promptLabel: 'system prompt 內容',
      promptPlaceholder: '直接寫入要傳給 LLM 的指令……',
      delete: '刪除',
      edit: '編輯',
      save: '儲存',
      cancel: '取消',
      viewBuiltinPrompt: '查看預設 prompt',
      hideBuiltinPrompt: '收起',
      confirmDelete: '確定刪除此自定義風格？',
      idEmpty: '請輸入 ID。',
      idInvalid: 'ID 只能包含英文字母、數字和連字號。',
      nameEmpty: '請輸入顯示名稱。',
      promptEmpty: '請輸入 system prompt。',
    },
    appOverride: {
      title: '依應用程式自動切換',
      desc: '在指定的應用程式中聽寫時，自動套用對應的風格。比對的是行程名稱的子字串（例如 chrome、Discord.exe），不區分大小寫；都不命中時採用預設風格。',
      addButton: '+ 新增規則',
      empty: '尚未設定規則。點擊「+ 新增規則」開始。',
      patternLabel: '應用程式名稱（子字串）',
      patternPlaceholder: '例：chrome / Discord.exe',
      modeLabel: '套用的風格',
      delete: '刪除',
      hint: '由上至下依序比對，最先命中的規則生效。比對不區分大小寫。',
    },
  },
  translation: {
    kicker: 'TRANSLATION',
    ...
    enable: {
      title: '啓用翻譯功能',
      hint: '關閉後，即使誤觸快捷鍵也不會啓動翻譯管線和 UI 浮層。',
    },
    working: {
      title: '工作語言',
  ...
  modal: {
    ...
    personalize: {
      ...
      fontMedium: '中',
      fontLarge: '大',
      fontFamilyLabel: 'UI 字體',
      fontFamilyDesc: '切換 App 內的顯示字體。適用於中日文字體渲染衝突或自訂偏好。',
      fontFamilyAuto: '自動（預設優先級）',
      fontFamilyCustomLabel: '自訂（自由輸入）…',
      fontFamilyCustomPlaceholder: '輸入字體名（例：Yu Gothic UI）',
      fontInstalledLabel: '已安裝字體 ({{count}})',
      fontLoadAll: '載入所有字體',
      fontReload: '重新載入 ({{count}})',
      fontLoading: '取得中…',
      fontLoadError: '無法取得字體列表（權限被拒或目前環境不支援）',
      quietLabel: '靜默模式',
      quietDesc: '保留錄音波形與處理點動畫，抑制「翻譯中」「已輸入 N 字」「已取消」等瞬時文字提示。錯誤仍會顯示。',
      quietAria: '切換靜默模式',
      blur: '毛玻璃強度',
