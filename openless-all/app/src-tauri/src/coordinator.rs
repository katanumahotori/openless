...
impl Coordinator {
    ...
    pub fn sync_active_asr_provider_from_preferences(&self) -> Result<(), String> {
        if CredentialsVault::has_stored_credentials_root() {
            log::info!(
                "[startup] keep active ASR provider from credentials vault; skip preferences backfill"
            );
            return Ok(());
        }
    ...
    pub async fn repolish(&self, raw_text: String, mode: PolishMode) -> Result<String, String> {
        ...
        let style_system_prompt = style_prompt_with_universal_directives(
            &pack.prompt,
