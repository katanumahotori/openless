...
#[cfg(test)]
pub(super) fn credentials_path() -> Result<PathBuf> {
    Ok(super::data_dir()?.join(LEGACY_CREDS_FILE))
}
...
fn credentials_root_is_meaningful(root: &CredsRoot) -> bool {
    legacy_vault_has_credentials(root)
        || root.active.asr != creds_default_asr()
        || root.active.llm != creds_default_llm()
}

fn stored_credentials_root_exists() -> Result<bool> {
    #[cfg(target_os = "android")]
    {
        return Ok(android_credentials_path()?.exists());
    }

    #[cfg(not(target_os = "android"))]
    {
        if load_keyring_credentials()?.is_some() {
            return Ok(true);
        }
        if let Some(legacy) = load_legacy_credentials() {
            return Ok(credentials_root_is_meaningful(&legacy));
        }
        let legacy_vault = load_legacy_keyring_credentials_for_update()?;
        Ok(credentials_root_is_meaningful(&legacy_vault))
    }
}
...
impl CredentialsVault {
    ...
    pub fn has_stored_credentials_root() -> bool {
        let _guard = credentials_lock().lock();
        match stored_credentials_root_exists() {
            Ok(exists) => exists,
            Err(e) => {
                log::warn!("[vault] stored credentials detection failed: {e}");
                true
            }
        }
    }
...
mod tests {
    use super::{
        chunk_json_payload, credentials_root_is_meaningful, CredsActive, CredsRoot,
    ...
    #[test]
    fn default_credentials_root_is_not_treated_as_user_choice() {
        assert!(!credentials_root_is_meaningful(&CredsRoot::default()));
    }

    #[test]
    fn explicit_active_asr_choice_is_treated_as_user_choice() {
        let mut root = CredsRoot::default();
        root.active = CredsActive {
            asr: "foundry-local-whisper".to_string(),
            ..CredsActive::default()
        };

        assert!(credentials_root_is_meaningful(&root));
    }
}
