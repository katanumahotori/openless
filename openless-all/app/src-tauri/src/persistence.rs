...
/// 每次 dictation 会话都会先写临时 WAV；成功/取消会话默认删除，失败会话保留用于恢复。
/// 用户开 `prefs.record_audio_for_debug` 时成功会话也保留。写入新文件时顺手按
/// `history_retention_days` / `audio_recording_max_entries` 裁旧。
